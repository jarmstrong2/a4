...
In both the relaxed linear program and rounding scheme function f will return the same results, such that the result of the relaxed linear program, since P is minimized, P will equal some P$_{m}$. Indeed suppose \mbox{P = P$_{m}$ = $\sum_{\forall q}$ x$_{q}^{f(q,~ m)}$}*. It follows, from our rounding scheme, that the rounding solution must be \mbox{P'$_{r}$ = $\sum_{\forall q}$ y$_{q}^{f(q,~ r)}$} such that r may or may not equal m. Then the proof is such that:\\
...
P'$_{r}$ = $\sum_{\forall q}$ y$_{q}^{f(q,~ m)}$ $\leqslant$
2*P$_{r}$ = 2*$\sum_{\forall q}$ x$_{q}^{f(q,~ r)}$\\ 
P'$_{r}$ = $\sum_{\forall q}$ y$_{q}^{f(q,~ m)}$
$\leqslant$
2*P$_{m}$ = 2*$\sum_{\forall q}$ x$_{q}^{f(q,~ m)}$~~~~~~~~\# Since P$_{r}$ $\leqslant$ P$_{m}$\\
P'$_{r}$ = $\sum_{\forall q}$ y$_{q}^{f(q,~ m)}$ $\leqslant$ 2*P $\leqslant$  2*LPOPT $\leqslant$ 2*IPOPT ~~~~~~~~\# Since we established that P = P$_{m}$\\
